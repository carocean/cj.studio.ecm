...
public class Assembly implements IAssembly, IAssemblyInfo, IClosable {
    ...
    protected IAssemblyContext createAssemblyContext(IResource resource) {
        ...
        Properties properties = findPropertiesFile(resource, home);
        File h = new File(String.format("%s/properties/Assembly.yaml", home));
        if (h.exists()) {
            CJSystem.logging().info(getClass(), String.format("发现程序集的外部上下文配置文件，路径：%s", h));
            InputStream stream = null;
            try {
                stream = new FileInputStream(h);
                Map<String, Object> node = new Yaml().load(stream);
                return new YamlAssemblyContext(resource, node, properties);
        ...
        h = new File(String.format("%s/properties/Assembly.json", home));
        if (h.exists()) {
            CJSystem.logging().info(getClass(), String.format("发现程序集的外部上下文配置文件，路径：%s", h));
            InputStream stream = null;
            try {
                stream = new FileInputStream(h);
                return new JsonAssemblyContext(resource, stream, properties);
            } catch (FileNotFoundException e) {
                throw new EcmException(e);
            } finally {
                if (stream != null) {
        ...
        String ctxConfFile = "cj/properties/Assembly.yaml";
        InputStream stream = resource.getResourceAsStream(ctxConfFile);
        if (stream != null) {
            Map<String, Object> node = new Yaml().load(stream);
            return new YamlAssemblyContext(resource, node, properties);
        }
        ctxConfFile = "cj/properties/Assembly.json";
        stream = resource.getResourceAsStream(ctxConfFile);
        if (stream == null) {
            throw new EcmException(String.format("未发现程序集上下文.at %s ",
                    resource.getResourcefile()));
        }
        return new JsonAssemblyContext(resource, stream, properties);
    }

    private Properties findPropertiesFile(IResource resource, String home) {
        File h = new File(String.format("%s/properties/Assembly.properties",
                home));
        if (h.exists()) {
            InputStream in = null;
            try {
                in = new FileInputStream(h);
                CJSystem.logging().info(getClass(), String.format("发现程序集的外部属性配置文件，路径：%s", h));
        ...
        InputStream in = resource.getResourceAsStream(IResource.CONTEXT_PROP_FILE);
        if (in == null) {
            throw new EcmException(String.format("未发现程序集属性配置文件.at %s ",
                    resource.getResourcefile()));
        }
        Properties props = new Properties();
        try {
