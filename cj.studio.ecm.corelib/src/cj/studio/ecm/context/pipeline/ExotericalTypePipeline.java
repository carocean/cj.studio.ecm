...
public class ExotericalTypePipeline implements IUpriverPipeline {
	...
	public ExotericalTypePipeline(IResource owner) {
	...
	private synchronized Class<?> searchReferenceType(String className,
			ExotericalTypePipeline pl) throws ClassNotFoundException {
		Class<?> c = null;
		for (IResource r : pl.referenceResources) {
			...
			if (c != null)
				...
				return searchReferenceType(className,
						(ExotericalTypePipeline) r.getPipeline());
			}
		}
		return c;
	}
