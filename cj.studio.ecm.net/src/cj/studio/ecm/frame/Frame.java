...
public class Frame implements IPrinter, IDisposable {
	...
	protected NettyContent createContent(int initLen) {
		ByteBuf buf = null;
	...
	public void fromJson(String text) {
		...
		if (conte != null) {
			content.writeBytes(conte.getAsString().getBytes());
		}
	}
	...
	public Frame copy() {
		Frame frame = null;
		if (this instanceof HttpFrame) {
			frame = new HttpFrame(this.toString());
		} else {
			frame = new Frame();
		}
		...
		frame.content = this.content.copyIt();
		return frame;
	}
	...
	public ByteBuf toByteBuf() {
		...
		b.writeBytes(crcf);
		b.writeBytes(ctxCopy);// 非常变态，bytebuf数组总是在结尾入多一个0，因此其长度总是比写入的长度多1个字节
		return b;
	}
