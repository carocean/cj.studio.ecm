...
public class HttpClientSync implements ISink, NetConstans {
	SyncPool pool;
	...
	public void flow(Frame frame,Circuit circuit, IPlug plug) throws CircuitException {
		if (plug.owner().startsWith("input$")) {
			if ("connect".equals(frame.command())) {
				throw NetConstans.newCircuitException(STATUS_611);
			}
			...
			if ("pull".equalsIgnoreCase(frame.command())
					|| "true".equals(frame.head(FRAME_HEADKEY_CIRCUIT_SYNC))) {
				// 如果是同步，则为之生成msgId
				...
				if (pool.put(msgId)) {
					...
					if (back != null) {
						...
						c.message("等待超时");
						String cause = String
								.format("原因是等待超时，超时时间：%s毫秒，默认是3800毫秒。\r\n可以在frame头中指定：syn-timeout，可以设置每侦的等待时间",
										syn_timeout);
		...
		if (plug.owner().startsWith("input_")) {
			String msgId = frame.head(FRAME_HEADKEY_FRAME_ID);
